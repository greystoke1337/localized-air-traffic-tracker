...
#define LVGL_PORT_BUFFER_MALLOC_CAPS            (MALLOC_CAP_SPIRAM)      // Allocate LVGL buffer in PSRAM (required for avoid-tearing full-frame buffers)
...
#else
#define LVGL_PORT_AVOID_TEARING_MODE            (3)     // Valid if using Arduino — mode 3: double-buffer + direct mode (required for RGB parallel LCD)
#endif
